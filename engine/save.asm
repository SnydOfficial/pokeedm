...
LoadSAVCheckSum: ; 73623 (1c:7623)
	...
	ld bc, $f8b + wEndOfExtraBytes - wBoxMonNicksEnd ; but here checks the full SAV
	call SAVCheckSum
	ld c, a
	...
	jp z, .Func_73652
	ld hl, $a598
	ld bc, $f8b + wEndOfExtraBytes - wBoxMonNicksEnd
	call SAVCheckSum
	ld c, a
	ld a, [$bfff] ; SAV's checksum
	cp c
...
.Func_73652 ; 73652 (1c:7652)
	...
	ld hl, $b0c0
	ld de, W_NUMINBOX
	ld bc, wEndOfExtraBytes - W_NUMINBOX
	call CopyData
	and a
	jp SAVGoodChecksum
...
;(which are stored at wPlayerID)
	...
	jr z,.next
	ld hl,$a598
	ld bc,$0f8b + wEndOfExtraBytes - wBoxMonNicksEnd
	call SAVCheckSum
	ld c,a
	ld a,[$bfff]
	cp c
